...
Feature: Launch an action if a rule is triggered in Perseo manager
  ...
  @happy_path
  Scenario: Launch a post action if a rule is triggered
    # Gen EPL
    ...
    Then the mock receive the number "1" of actions "post"
    And the mock receive this part of text "the new temperature is 300" in the action "post"