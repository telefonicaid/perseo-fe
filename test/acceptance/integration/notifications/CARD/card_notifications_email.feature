...
Feature: Launch an action if a visual rule is triggered in Perseo manager
  ...
  @happy_path @email_value_threshold_card
  Scenario: Launch an email action if a visual rule is satisfied using only a threshold card and an action card
    ...
    And create an email action card with id "card_5" from "from@from.com" to "to@to.com" subject "subuject_example" body "the new temperature is ${temperature}" and connected to ""
    And append a new rule name "email_rule", activate "1"
    # notifications
    ...
    And the mock receive this part of text "the new temperature is 300" in the action "email"
