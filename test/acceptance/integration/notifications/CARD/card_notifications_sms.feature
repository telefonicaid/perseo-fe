...
Feature: Launch an action if a visual rule is triggered in Perseo manager
  ...
  Scenario: Launch an sms action if a visual rule is satisfied using only a threshold card and an action card
    ...
    And create a sms action card with id "card_5" text "the new temperature is ${temperature}" telephone number "666999666" and connected to ""
    And append a new rule name "sms_rule", activate "1"
    # notifications
