...
Late 2018 `-ficodes` versions of Perseo should work with any of the EPL clauses. However, you should take into
...
Perseo triggers the action, the process may repeat, i.e. first update attemp fails with 401, Perseo requests a fresh
auth token to Keystone, the second update attemp fails with 401, Perseo logs the problem and doesn't retry again.
