...
| PERSEO_MAX_RULES_BY_CORR | Maximun number of rules triggered by the some notification (correlator) until refuse execute a rule. Default is 20. See [loop detection tips](https://github.com/telefonicaid/perseo-fe/blob/master/docs/architecture/architecture.md#loop-detection)
|
...
-   `config.maxRulesByCorr`: Maximum number of rules triggered by the some notification (correlator) until refuse execute a rule. Defualt is 20
