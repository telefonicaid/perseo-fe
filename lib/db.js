...
var async = require('async'),
    ...
    database,
    orionDb,
    delay = 2000, reportInterval = 15e3, lastErrorTrace = 0;
...
function ensureIndex(collection, fields, callback) {
    database.collection(collection, function(err, collection) {
        myutils.logErrorIf(err, 'collection rules');
...
function setUp(cbSU) {
    async.parallel([
        ensureIndex.bind(null, rulesCollection, {name: 1, service: 1, tenant: 1}),
        ensureIndex.bind(null, executionsCollection, {id: 1, name: 1, service: 1, tenant: 1})
    ], cbSU);
}
