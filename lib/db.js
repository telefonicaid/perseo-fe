...
function pingAux(db, component, callback) {
    db.command({ping: 1}, function(err, result) {
        myutils.logErrorIf(err, 'ping', context);
        if (err) {
...
function getDbAux(url, component, callback) {
    ...
    client.connect(url, function(err, db) {
        ...
        dbErrorFunc = function dbErrorFunc(err) {
            if (err) { // Should, this is an error handler
                var now = Date.now();
                if (lastErrorTrace + reportInterval < now) {
                    myutils.logErrorIf(err, 'database error ' + url, context);
                    lastErrorTrace = now;
                }
                alarm.raise(component, context, err);
            }
        };
