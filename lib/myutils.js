...
function ruleWithContext(rule) {
    const preSelectMatch = rule.text.match(/.+?(?=select)/i);
    const SelectMatch = rule.text.match(/select.*/i);
    const insertionPreSelect = preSelectMatch ? preSelectMatch[0] : '';
    const insertionSelect = SelectMatch ? SelectMatch[0] : '';

    return removeCRLFchars(util.format('%s context %s %s', insertionPreSelect, contextName(rule), insertionSelect)).trim();
}
...
function ruleWithContextTimedRule(rule) {
    ...
    return removeCRLFchars(util.format('%s context %s %s', insertionPreSelect, contextNameTimedRule(rule), insertionSelect)).trim();
}
