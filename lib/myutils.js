...
function logErrorIf(err, message, level) {
    if (err) {
        level = level || 'error';
        message = message || '';
        if (process.domain) {
            process.domain.op = process.domain.op || new Error().stack.split('\n')[2].trim().substr(3);
        }
        logger[level](message, err.message || err.toString());
    }
...
function requestHelper(method, options, callback) {
    var localError, respObj, headers, domain = require('domain').active;
    logger.info('making %s to %s', method, options.url);
    headers = options.headers || {};
    if (domain && domain.corr) {
        headers[constants.CORRELATOR_HEADER] = domain.corr;
    }
    ...
    request[method](options, function cbRequest2core(err, response, body) {
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
            localError = new Error(util.format('error %s to %s (%s)', method, options.url,
                (body && body.error)  || body || response.statusCode));
            localError.httpCode = 500;
            logErrorIf(localError);
...
function contextName(rule) {
    return util.format('ctxt$%s%s', purgeName(rule.tenant), purgeName(rule.service));
}

function ruleUniqueName(rule) {
    return util.format('%s@%s%s', rule.name, rule.tenant, rule.service);
}

function contextEPL(rule) {
    return util.format('create context %s partition by tenant from iotEvent(tenant="%s" and service="%s")',
        contextName(rule), rule.tenant, rule.service);
}
...
/**
 * ruleUniqueName returns a unique name for a rule, including service and tenant names
 *
...
/**
 * contextRuleText returns the EPL text for creating a context for service and tenant names
 *
 * @param  {Object} Object rule
 */
...
/**
 * contextRuleName returns the name of a context for service and tenant names
 *
 * @param  {Object} Object rule
 */
