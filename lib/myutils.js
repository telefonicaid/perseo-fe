...
function logErrorIf(err, message, context) {
    ...
    if (contextTemp === undefined) {
        contextTemp = process.domain && process.domain.context;
    }
    if (err) {
        messageTemp = message || '';
        if (contextTemp) {
            contextTemp.op =
                ...
                new Error().stack
                    ...
                    .trim()
                    .substr(3);
            contextTemp.comp = contextTemp.comp || constants.COMPONENT_NAME;
            logger[level](context, messageTemp, err.message || JSON.stringify(err));
        } else {
            logger[level](messageTemp, err.message || JSON.stringify(err));
        }
    }
}
...
function expandVar(val, mapping, trycast) {
    var valTemp = val;
    if (typeof valTemp === 'string') {
        Object.keys(mapping).forEach(function(p) {
            valTemp = valTemp.replace(new RegExp('\\$\\{' + p + '\\}', 'g'), mapping[p]);
        });
        var expanded = false;
        if (trycast) {
            try {
                // Check if "${num}" and expand it as real value, non string
                var n = JSON.parse(valTemp);
                switch (typeof n) {
                    case 'number':
                    case 'boolean':
...
function respondAux(resp, err, data, withCount, raw, withMetrics) {
    ...
    if (raw === true) {
        ...
        // non-raw
        respObj = { error: errMsg, data: dataTemp };
        if (withCount === true && dataTemp && util.isArray(dataTemp)) {
            respObj.count = dataTemp.length;
        }
    }
...
function firstChars(result) {
    ...
    text = JSON.stringify(resultTemp);
    if (text.length > 125) {
