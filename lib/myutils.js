...
function ruleWithContext(rule) {
    ...
    const insertionPreContext = preInsertMatch ? preInsertMatch[0] : preSelectMatch ? preSelectMatch[0] : null;
    // PostContext
...
function ruleWithContextTimedRule(rule) {
    ...
    const preSelectMatch = ruleText.match(/.+?(?=\bselect )/i);
    const preInsertMatch = preSelectMatch ? preSelectMatch[0].match(/.+?(?=\insert )/i) : null;
    const insertionPreContext = preInsertMatch ? preInsertMatch[0] : preSelectMatch[0];
    // PostContext
    const InsertMatch = ruleText.match(/\binsert .*/i);
    var insertionPostContext;
    if (InsertMatch) {
