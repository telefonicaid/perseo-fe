...
function ruleWithContext(rule) {
    ...
    const insertionPreInsert = preInsertMatch ? preInsertMatch[0] : '';
    ...
    const insertionInsert = InsertMatch ? InsertMatch[0] : insertionSelect;
    return util.format('%s context %s %s', insertionPreInsert, contextName(rule), insertionInsert).trim();
}
...
function ruleWithContextTimedRule(rule) {
    ...
    const insertionSelect = SelectMatch ? SelectMatch[0] : '';
    const insertionInsert = InsertMatch ? InsertMatch[0] : insertionSelect;
    return util.format('%s context %s %s', insertionPreInsert, contextNameTimedRule(rule), insertionInsert).trim();
}
