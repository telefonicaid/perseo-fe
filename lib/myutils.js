...
function expandObject(templateObj, dictionary) {
    if (Array.isArray(templateObj)) {
        // If array then map each element recursively
        return templateObj.map((item) => expandVar(item, dictionary, true));
    } else if (templateObj && typeof templateObj === 'object') {
        // If an object then process key as usually
        var res = {};
    ...
    // If not array or object (for example string or number), is returned as is
    return templateObj;
}

// Do not flat into a map object or array attributes
...
function respondAux(resp, err, data, withCount, raw, withMetrics) {
    ...
    if (raw === true) {
        ...
        if (withCount === true && data && Array.isArray(data)) {
...
function ruleWithContext(rule) {
    ...
    if (preSelectMatch) {
        if (InsertMatch) {
            ...
            insertionPreContext = preSelectMatch[0];
        }
    } else {
        preInsertMatch = null;
        insertionPreContext = '';
    ...
    if (InsertMatch) {
        insertionPostContext = InsertMatch[0];
    } else {
        const SelectMatch = ruleText.match(/\bselect .*/i);
        insertionPostContext = SelectMatch ? SelectMatch[0] : '';
    }
    // Put all toghther with a context:
    //   <Expression E ZZZ> Context <insert into XXX> select YYY
    return util.format('%s context %s %s', insertionPreContext, contextName(rule), insertionPostContext).trim();
}
...
function ruleWithContextTimedRule(rule) {
    ...
    //   <Expression E ZZZ> Context <insert into XXX> select YYY
    return util
        .format('%s context %s %s', insertionPreContext, contextNameTimedRule(rule), insertionPostContext)
        .trim();
}

function collectionExists(db, colName, callback) {
    db.listCollections({ name: colName }, { nameOnly: true })
        .toArray()
        .then(function(collections) {
            callback(collections.length > 0);
        })
        .catch(function() {
            callback(false);
        });
...
/**
 * collectionExists checks if a given collection exists (to get in mongodb 4.x the same "strict mode"
 * we had in mongodb 3.x, this way achieving full backward compatibility). Check also
 * https://github.com/mongodb/node-mongodb-native/blob/HEAD/etc/notes/CHANGES_4.0.0.md#dbcollection-no-longer-accepts-a-callback
 *
 * @param {object}       db
 * @param {string}       colName
 * @param {function}     callback function(found)
 */
