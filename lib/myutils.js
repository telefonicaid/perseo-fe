...
function contextName(rule) {
    return util.format('ctxt$%s%s', purgeName(rule.tenant), purgeName(rule.subservice));
}

function ruleUniqueName(rule) {
    return util.format('%s@%s%s', rule.name, rule.tenant, rule.subservice);
}

function contextEPL(rule) {
    return util.format('create context %s partition by tenant from iotEvent(tenant="%s" and subservice="%s")',
        contextName(rule), rule.tenant, rule.subservice);
}
...
/**
 * ruleUniqueName returns a unique name for a rule, including subservice and tenant names
 *
...
/**
 * contextRuleText returns the EPL text for creating a context for subservice and tenant names
 *
 * @param  {Object} Object rule
 */
...
/**
 * contextRuleName returns the name of a context for subservice and tenant names
 *
 * @param  {Object} Object rule
 */
