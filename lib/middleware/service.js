...
exports.middleware = function() {
    return function serviceNtenant(req, res, next) {
        var subservice = req.headers[constants.SERVICE_HEADER],
            service = req.headers[constants.TENANT_HEADER],
            err;
        if (!subservice) {
            logger.debug('missing subservice header');
            subservice = config.DEFAULT_SERVICE;
        }
        else {
        ...
        req.service = subservice.toLowerCase();
        req.tenant = service.toLowerCase();
        next();
    };
};
