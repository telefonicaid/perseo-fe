...
function search(rule, callback) {
    ...
    async.waterfall([
        ...
        function(col, cb) {
            col.findOne({name: rule.name, subservice: rule.subservice, tenant: rule.tenant}, cb);
        }
    ], callback);
}

function findAll(tenant, subservice, callback) {
    var criterion = {};
...
module.exports = {
    ...
    Remove: function Remove(rule, callback) {
        ...
        async.waterfall([
            db.collection.bind(db, rulesCollectionName, {strict: true}),
            function(col, cb) {
                col.remove({name: rule.name, subservice: rule.subservice, tenant: rule.tenant}, cb);
            }
        ], function(err, result) {
            myutils.logErrorIf(err);
