                ...
                function(callbackS) {
                    switch (action.type) {
                        case 'email':
                            ...
                            break;
                        case 'sms':
                            if (action.parameters && action.parameters.sms) {
                                smsAction.doIt(action, event, callbackS);
                            } else if (action.parameters && action.parameters.smpp) {
...
function slaveExec(task, alreadyExecuted, callback) {
    logger.debug('slaveExec alreadyExecuted=%j task %j', alreadyExecuted, task);
    if (!alreadyExecuted) {
...
function DoAction(event, callback) {
    ...
    actionStore.Find(service, subservice, ruleName, function(err, actions) {
        ...
        for (i = 0; i < actions.length; i++) {
            actions[i].index = i;
            logger.debug('inserting in queue action %j', actions[i]);
            queue.push({
                action: actions[i],
                event: event,
