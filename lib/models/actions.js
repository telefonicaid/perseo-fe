...
function conditionalExec(action, event, lastTime, callbackW) {
    ...
    if (lastTime + action.interval < rightNow) {
        async.series([
            ...
            executionsStore.Update.bind(null, tenant, subservice, ruleName, eventId, noticeId)
        ], callbackW);
    }
    else {
...
function slaveExec(action, event, alreadyExecuted, callback) {
    ...
    if (!alreadyExecuted) {
        async.waterfall(
            [
                executionsStore.LastTime.bind(null, tenant, subservice, ruleName, eventId),
...
function execAxn(action, event, cbCe) {
    ...
    if (config.isMaster) {
        ...
        async.waterfall(
            [
                executionsStore.AlreadyDone.bind(null, tenant, subservice, ruleName, eventId, noticeId),
...
function DoAction(event, callback) {
    ...
    var ruleName = event.ruleName,
        ...
        tenant = event.tenant,
        localError;
    logger.debug('tenant %s, subservice %s, event %j', tenant, subservice, event);
    localError = errorsAction(event);
    if (localError !== null) {
        myutils.logErrorIf(localError);
        return callback(localError, null);
    }
    actionStore.Find(tenant, subservice, ruleName, function(err, action) {
        if (err) {
