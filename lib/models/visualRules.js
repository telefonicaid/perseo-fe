...
function castVar(str, isNumber) {
    var castStr = util.format('cast(`%s`?, String)', str);

    if (isNumber === true) {
        castStr = util.format('cast(%s, float)', castStr);
    }
    return castStr;
}
...
function adaptExp(str, isNumber) {
    var adaptString = str;
    if (adaptString.indexOf('${') !== 0) {
        if (isNumber !== true) {
            adaptString = util.format('%j', adaptString);
        }
        return adaptString;
    }
    //Remove starting "${" and trailing "}"
    adaptString = adaptString.slice(2, -1);
    //Change dot to double underscore
    adaptString = adaptString.replace(/\./g, '__');
    return castVar(adaptString, isNumber);
}

function makeUserPropCondition(exprL, op, exprR, isNumber) {
    var adaptExprL = adaptExp(exprL, isNumber);
    var adaptExprR = adaptExp(exprR, isNumber);
    return util.format('(%s %s %s)', adaptExprL, operatorMap[op], adaptExprR);
}
function makeObservationCondition(exprL, op, exprR, isNumber) {
    var castExprL = castVar(exprL, isNumber);
    var adaptExprR = adaptExp(exprR, isNumber);
    return util.format('(%s %s %s)', castExprL, operatorMap[op], adaptExprR);
}
...
module.exports = {
    ...
    Put: function(id, rule, callback) {
        var reformatRule = vr2rule(rule);
        if (reformatRule instanceof Error) {
            myutils.logErrorIf(reformatRule);
            return callback(reformatRule, null);
        }
        rules.Put(id, reformatRule, function(err, data) {
            if (err) {
                return callback(err, null);
            }
