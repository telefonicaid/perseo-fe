...
function transformAxnParam(str) {
    str = str.replace(/\$\{([A-Za-z0-9_.]*)\}/g, function(m, g1) {
        return util.format('${ev__%s}', g1.replace(/\./g, '__'));
    });
    return str;
}
        ...
        for (i = 0; i < cardRule.cards.length; i += 1) {
            ...
            case 'SensorCard':
                ...
                case 'OBSERVATION':
                    ...
                    break;
                case 'LAST_MEASURE':
                    if (isNaN(parseInt(card.timeData.interval, 10))) {
                        return new errors.InvalidVisualRule('invalid period time ' + card.timeData.interval);
                    }
                    noSignal = {
            ...
            case 'ActionCard':
                ...
                case 'SendEmailAction':
                    ...
                    action.type = 'email';
                    action.template = transformAxnParam(plain['mail.message']);
                    action.parameters = {
                        to: plain['mail.to'],
                ...
                case 'SendSmsMibAction':
                    plain = plainParams(card.actionData.userParams);
                    action.type = 'sms';
                    action.template = transformAxnParam(plain['sms.message']);
                    action.parameters = {
                        to: plain['sms.to']
                    };
                    break;
...
module.exports = {
    FindAll: function FindAll(tenant, service, callback) {
        rules.FindAll(tenant, service, function(err, data) {
            if (err) {
...
(function() {
    ...
    errors.MissingConfigDataInTimeCard = function MissingConfigDataInTimeCard(msg) {
        this.name = 'MISSING_CONGIDATA_TIMECARD';
