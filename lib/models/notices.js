 ...
 *
 * @param locStr The location string e.g: "40.418889, -3.691944" (lat, lon)
 *
    ...
    try {
        ...
        for (var attr in attrList) {
            ...
            if (attrInfo.type === 'geo:point') {
                ...
                // copy an reverse array due to parseLocation expect lat,lon and a geo:json Point array is [lon,lat]
                location = parseLocation(
                    attrInfo.value.coordinates
                        .slice()
                        .reverse()
                        .toString()
                );
            ...
            for (var metaKey in attrInfo.metadata) {
                ...
                if (attrInfo.metadata[metaKey].type === 'DateTime') {
                    ...
                    // copy an reverse array due to parseLocation expect lat,lon and a geo:json Point array is [lon,lat]
                    metalocation = parseLocation(
                        attrInfo.metadata[metaKey].value.coordinates
                            .slice()
                            .reverse()
                            .toString()
                    );
                ...
                if (metalocation !== null) {
                    ...
                    if (!(metalocation instanceof Error)) {
                        n[attr + '__metadata__' + metaKey + '__lat'] = metalocation.lat;
                        n[attr + '__metadata__' + metaKey + '__lon'] = metalocation.lon;
                        n[attr + '__metadata__' + metaKey + '__x'] = metalocation.x;
                        n[attr + '__metadata__' + metaKey + '__y'] = metalocation.y;
                        metalocation = null;
                    }
                }
            }
