...
try {
    ...
    logger.error('Error %s loading configTrust.js ', err);
}
...
function watchConfigTrustFile() {
    fs.watch(configTrustFilePath, (event, filename) => {
        ...
        try {
            if (!fs.existsSync(configTrustFilePath)) {
                logger.error('Error %s file was removed', filename);
                return;
            }
            const newConfig = requireUncached('../../configTrust.js');
            if (
                !newConfig ||
                !newConfig.configTrust ||
                !newConfig.configTrust.trusts ||
                !Array.isArray(newConfig.configTrust.trusts)
            ) {
                logger.error('Error configTrust not in expected format');
                return;
            }
            configTrust = newConfig.configTrust;
