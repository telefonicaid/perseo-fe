...
module.exports = {
    LastTime: function LastTime(tenant, service, ruleName, id, callback) {
        ...
        db.collection(execCollectionName, {strict: true}, function(err, col) {
            ...
            col.findOne({'name': ruleName, 'service': service, 'tenant': tenant, 'id': id}, function(err, data) {
                myutils.logErrorIf(err);
                if (err) {
                    return callback(err, null);
                }
                return callback(null, (data && data.lastTime) || 0);
            });
        });
    },
    Update: function Update(tenant, service, ruleName, id, callback) {
        var db = appContext.Db();
        async.waterfall([
            db.collection.bind(db, execCollectionName, {strict: true}),
            function(col, cb) {
                col.update({'name': ruleName, 'service': service, 'tenant': tenant, 'id': id},
