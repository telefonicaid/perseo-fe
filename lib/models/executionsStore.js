...
module.exports = {
    LastTime: function LastTime(service, subservice, ruleName, id, callback) {
        ...
        db.collection(execCollectionName, {strict: true}, function(err, col) {
            ...
            col.findOne({
                '$query': {'name': ruleName, 'subservice': subservice, 'service': service, 'id': id},
                '$orderby': {'lastTime': -1}
            }, function(err, data) {
                myutils.logErrorIf(err);
                if (err) {
    ...
    AlreadyDone: function AlreadyDone(service, subservice, ruleName, id, noticeId, callback) {
        var db = appContext.Db();
        db.collection(execCollectionName, {strict: true}, function(err, col) {
            if (err) {
                myutils.logErrorIf(err);
            ...
            col.findOne({'name': ruleName, 'subservice': subservice, 'service': service, 'id': id, 'notice': noticeId},
                function(err, data) {
                    myutils.logErrorIf(err);
                    if (err) {
                        return callback(err, null);
    ...
    Update: function Update(service, subservice, ruleName, id, noticeId, callback) {
        var db = appContext.Db();
        async.waterfall([
