...
function doIt(action, event, callback) {
    try {
        ...
        var sendSMPP = function(from, to, text) {
            ...
            // from & to are integers. We need to convert them to String
            // and add `+` before
            from = '+' + from.toString();
            to = '+' + to.toString();
            ...
            session.submit_sm({
                ...
                logger.debug('sendSMPP submit_sm');
                //logger.debug('smpp sms pdu status %s', lookupPDUStatusKey(pdu.command_status));
        ...
        session.on('connect', function() {
            /*jshint camelcase:false */
            didConnect = true;
            logger.debug('smpp session on connect system_id: %s, password: %s', config.smpp.systemid, config.smpp.password);
            session.bind_transceiver({
                system_id: config.smpp.systemid,
                password: config.smpp.password
            }, function(pdu) {
                logger.debug('sendSMPP bind_transceiver');
                //logger.debug('pdu status', lookupPDUStatusKey(pdu.command_status));
                if (pdu.command_status === 0) {
                    logger.debug('SMPP Successfully bound');
        ...
        session.on('error', function(error) {
            logger.debug('smpp error %j', error);
            didConnect = false;
        });
