...
function buildSMSOptions(action, event) {
    ...
    if (action.parameters.smpp) {
        if (action.parameters.smpp.from) {
            options.from = myutils.expandVar(action.parameters.smpp.from, event);
        }
        if (action.parameters.smpp.host) {
            options.host = myutils.expandVar(action.parameters.smpp.host, event);
        }
        if (action.parameters.smpp.port) {
            options.port = myutils.expandVar(action.parameters.smpp.port, event);
        }
        if (action.parameters.smpp.systemid) {
            options.systemid = myutils.expandVar(action.parameters.smpp.systemid, event);
        }
        if (action.parameters.smpp.password) {
            options.password = myutils.expandVar(action.parameters.smpp.password, event);
        }
    }
    return options;
}
