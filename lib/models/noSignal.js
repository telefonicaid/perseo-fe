...
function checkNoSignal(period) {
    ...
    list.forEach(function(nsrule, i) {
        entitiesStore.FindSilentEntities(nsrule[TENANT], nsrule[SUBSERVICE],
...
function nsr2arr(tenant, subservice, name, nsr) {
    var arrayRule = [];
...
function addNSRule(tenant, subservice, name, nsr) {
    var arrayRule,
        alreadyExists = false;
    arrayRule = nsr2arr(tenant, subservice, name, nsr);
    ...
    nsRulesByInterval[nsr.checkInterval].forEach(function(element, index, array) {
        if (element[NAME] === name && element[TENANT] === tenant && element[SUBSERVICE] === subservice) {
            array[index] = arrayRule; //Update rule
            alreadyExists = true;
            logger.debug(context, util.format('Updating no-signal rule (%s, %s, %s)', tenant, subservice, name));
        }
    });
    if (!alreadyExists) {
        nsRulesByInterval[nsr.checkInterval].push(arrayRule);
        logger.debug(context, util.format('Adding no-signal rule (%s, %s, %s)', tenant, subservice, name));
    }
    ...
    Object.keys(nsRulesByInterval).forEach(function(chkInt) {
        ...
        line.forEach(function(element, index) {
            if (element[NAME] === name && element[TENANT] === tenant && element[SUBSERVICE] === subservice) {
                ...
                logger.debug(context, util.format('Deleting no-signal rule (%s, %s, %s)', tenant, subservice, name));
            }
        });
    });
}
...
function refreshAllRules(rules) {
    ...
    rules.forEach(function(rule) {
        if (rule.nosignal) {
            addNSRule(rule.tenant, rule.subservice, rule.name, rule.nosignal);
            count++;
        }
    });
