...
function refreshAllRules(rules) {
    ...
    rules.forEach(function(rule) {
        if (rule.nosignal) {
            //check if any checkInterval has been modified.
            old_rule = getNSRule(rule.service, rule.subservice, rule.name);
            //deleteNSRuleIf(function(srv,subsrv, name){
            //  return old_rule[CHECK_INTERVAL]!=rule.nosignal.checkInterval;
            //});
            if(old_rule[CHECK_INTERVAL]!=rule.nosignal.checkInterval) {
              //the interval has been modified. DeleteNSRule to old_rule
              DeleteNSRule(old_rule[SERVICE], old_rule[SUBSERVICE], old_rule[NAME]);
            }
