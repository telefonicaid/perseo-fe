 ...
 */
function getUpdateConnection(action, event, token) {
    var options = {
    ...
    if (pep) {
        ...
        connection = new NGSI.Connection(config.orion.URL, options);
    }
    return connection;
}
    ...
    if (action && action.parameters && action.parameters.filter) {
        ...
        connection.v2.listEntities(queryOptions).then(
            (response) => {
                ...
                //
                connection = getUpdateConnection(action, event, token);
