...
function buildUpdateOptions(action, event) {
    ...
    if (action.parameters.actionType) {
        logger.debug('actionType: %j ', action.parameters.actionType);
        options.actionType = myutils.expandVar(action.parameters.actionType, event) || 'APPEND';
        logger.debug('actionType expanded: %j ', myutils.expandVar(action.parameters.actionType, event));
    }
    options.attributes = [];
