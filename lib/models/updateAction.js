...
function buildUpdateOptions(action, event) {
    ...
    if (myutils.expandVar(action.parameters.updateType, event)) {
        options.updateType = myutils.expandVar(action.parameters.updateType, event);
    }
    options.attributes = [];
...
function doRequest(action, event, token, callback) {
    ...
    if (options.updateType !== undefined) {
        updateOrion.updateAction = options.updateType;
    }
