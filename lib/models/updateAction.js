    ...
    if (action && action.parameters && action.parameters.filter) {
        ...
        do {
            if (queryOptions.iterateAll) {
                ...
                queryOptions.offset = queryOptions.offset ? queryOptions.offset : currentOffset;
            }
            //
            // 1. Make query with filter (default limit is 20)
            //
                    ...
                    if (changes.entities.length > 0) {
                        ...
                        connection.v2.batchUpdate(changes).then(
                            (updateResponse) => {
                                ...
                                // updateResponse.correlator transaction id associated with the server response
                                logger.debug('response batchUpdate: %j', updateResponse);
                                metrics.IncMetrics(event.service, event.subservice, metrics.okActionEntityUpdate);
                                alarm.release(alarm.ORION);
                                // update current response with previous response values for count and offset
                        ...
                        // logger.debug()
                        logger.debug('no entities to update in a batchUpdate: %j', response);
