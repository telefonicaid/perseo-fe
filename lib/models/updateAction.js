...
function processUpdateOptionParams(action, entity) {
    ...
    var options = {};
    action.parameters.attributes.forEach(function(attr) {
        let theValue = attr.value !== undefined ? attr.value : attr.name in entity ? entity[attr.name].value : '';
        let theType = attr.type !== undefined ? attr.type : attr.name in entity ? entity[attr.name].type : '';
        // Metadata should be null or object
        let theMeta = attr.metadata;
        var date;
