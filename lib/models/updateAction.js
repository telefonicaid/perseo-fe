 ...
 *
 */
function doUpdateActionWithFilter(action, event, token, connection, callback) {
    // UpdateAction with filter
    logger.debug('action with filter: %j, event: %j', action, event);
...
function doRequestV2(action, event, token, callback) {
    ...
    if (action && action.parameters && action.parameters.filter) {
        doUpdateActionWithFilter(action, event, token, connection, callback);
    } else {
