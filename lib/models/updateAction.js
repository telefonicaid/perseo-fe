    ...
    myutils.requestHelper(
        ...
        function(err, data) {
            ...
            if (!err && data.body && data.body.orionError) {
                var errOrion = new errors.OrionError(JSON.stringify(data.body.orionError));
                callback(errOrion, data);
            } else {
                callback(err, data);
            }
        }
...
function makeTokenListenerFunc(action, event, callback) {
    return function tokenHandlerFunc(error, token) {
        ...
        if (error || !token) {
            ...
            return doRequest(action, event, token, function cbDoReqUpdAxn(err, data) {
                callback(err, data);
            });
        }
    };
...
function doRequestV2(action, event, callback) {
    ...
    // NGSIv2 update action with trust token (similar to doItWithToken for NGSIv1)
