...
function validNameRule(rule) {
    let err = null;
    if (!rule || !rule.text || !rule.name) {
        return err;
    }
    const match = rule.text.match(/select\s+"([^"]+)"\s+as\s+ruleName/i);
    if (match) {
        const ruleNameInText = match[1];
        if (ruleNameInText !== rule.name) {
            err = new errors.InvalidRuleNameInEPL(rule.name);
        }
    }
    return err;
}
...
function normalizeRuleName(rule) {
    ...
    var hasRuleName = /\bas\s+ruleName\b/i.test(rule.text);
    // Add "name as ruleName" if not exist
    if (!hasRuleName) {
        var match = rule.text.match(/\bselect\b/i);
        if (match) {
            var idx = match.index + match[0].length;
            rule.text = rule.text.slice(0, idx) + ' ' + newAs + ', ' + rule.text.slice(idx).trim();
        }
    }
    return rule;
}
...
module.exports = {
    ...
    normalizeRuleName: normalizeRuleName
};
/**
...
(function() {
    ...
    errors.InvalidRuleNameInEPL = function InvalidRuleNameInEPL(msg) {
        this.name = 'INVALID_RULE_NAME_IN_EPL';
        this.message = 'invalid name in EPL text, expecting ' + msg;
        this.httpCode = 400;
    };
