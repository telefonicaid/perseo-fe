...
function normalizeRuleName(rule) {
    ...
    if (rule.text) {
        ...
        var asx = rule.text.indexOf(' as ruleName');
        if (asx === -1) {
            // Add "name as ruleName" if not exist
            rule.text = rule.text = rule.text.slice(0, idx) + newAs + ', ' + rule.text.slice(idx);
        } else {
            // Force to use rule.name in EPL text when as ruleName exists
            rule.text = rule.text.slice(0, idx) + '"' + rule.name + '"' + rule.text.slice(asx);
        }
    }
...
module.exports = {
    ...
    Put: function(id, rule, callback) {
        ...
        // Normalize the rule text
        logger.debug('put rule before normalized: %j', rule);
        rule = normalizeRuleName(rule);
        logger.debug('put rule after normalized: %j', rule);
