...
function validRule(rule){
    ...
    if (typeof rule.name !== 'string') { //includes null ('object') and undefined ('undefined')
        err =  new errors.MustBeStringRuleName(typeof rule.name);
    } else if (rule.name.length === 0) { //empty string ""
        err =  new errors.MissingRuleName(JSON.stringify(rule.name));
    } else if (rule.name.length > MaxNameLength) {
        err =  new errors.TooLongRuleName(JSON.stringify(rule.name));
    } else if (!namePattern.test(rule.name)){
        err = new errors.InvalidRuleName(rule.name);
    }
    else if (!rule.text && !rule.nosignal) {
        err = new errors.EmptyRule(JSON.stringify(rule));
    }
    return err;
}
