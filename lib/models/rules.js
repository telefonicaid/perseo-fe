...
module.exports = {
    ...
    Save: function(rule, callback) {
        ...
        normalizeRule = normalizeRuleName(rule);

        async.series(
            [
                function(localCallback) {
                    rulesStore.Exists(normalizeRule, function rsSaveExistsCb(err, exists) {
                        if (err) {
                            return localCallback(err);
                        }
                        if (exists) {
                            return localCallback(new errors.RuleExists(normalizeRule.name));
                        }
                        return localCallback(err, exists);
                    });
                },
                postR2core.bind(null, normalizeRule),
                rulesStore.Save.bind(null, normalizeRule),
                function(cb) {
                    if (rule.nosignal) {
                        noSignal.AddNSRule(
                            normalizeRule.service,
                            normalizeRule.subservice,
                            normalizeRule.name,
                            normalizeRule.nosignal
                        );
                    }
                    cb(null);
                }
    ...
    Put: function(id, rule, callback) {
        ...
        // Normalize the rule text
        var putNormalizeRule = normalizeRuleName(rule);

        async.series(
            [
                delR2core.bind(null, putNormalizeRule),
                function(cb) {
                    if (rule.nosignal) {
                        noSignal.DeleteNSRule(
                            putNormalizeRule.service,
                            putNormalizeRule.subservice,
                            putNormalizeRule.name
                        );
                    }
                    cb(null);
                },
                postR2core.bind(null, putNormalizeRule),
                function(cb) {
                    if (putNormalizeRule.nosignal) {
                        noSignal.AddNSRule(
                            putNormalizeRule.service,
                            putNormalizeRule.subservice,
                            putNormalizeRule.name,
                            putNormalizeRule.nosignal
                        );
                    }
                    cb(null);
                },
                rulesStore.Update.bind(null, id, putNormalizeRule)
            ],
            callback
        );
    }
