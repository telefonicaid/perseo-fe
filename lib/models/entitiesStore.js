...
function findSilentEntities(service, subservice, ruleData, alterFunc, callback) {
    if (!config.nonSignalByAPI) {
        return findSilentEntitiesByMongo(service, subservice, ruleData, alterFunc, callback);
    } else {
        return findSilentEntitiesByAPI(service, subservice, ruleData, alterFunc, callback);
    }
}
