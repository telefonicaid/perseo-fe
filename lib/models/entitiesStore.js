...
var async = require('async'),
    ...
    ngsi = require('ngsijs'),
    context = { op: 'checkNoSignal.findSilentEntitiesByAPI', comp: constants.COMPONENT_NAME };
...
function findSilentEntitiesByAPI(service, subservice, ruleData, alterFunc, callback) {
    ...
    findSilentEntitiesByAPIWithPagination(connection, filter, alterFunc, total, (err, results) => {
        if (err) {
            callback(err, null);
        } else {
...
function findSilentEntities(service, subservice, ruleData, alterFunc, callback) {
    ...
    if (!config.nonSignalByAPI) {
        logger.debug(context, 'Executing findSilentEntitiesByMongo');
        return findSilentEntitiesByMongo(service, subservice, ruleData, alterFunc, timedCallback);
    } else {
        logger.debug(context, 'Executing findSilentEntitiesByAPI');
