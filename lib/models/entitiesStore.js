...
function findSilentEntitiesByAPI(service, subservice, ruleData, func, callback, limit = 20, offset = 0) {
    var context = { op: 'checkNoSignal.findSilentEntitiesByAPI', comp: constants.COMPONENT_NAME };

    // Create the connection using the provided service and subservice
    var connection = createConnection(service, subservice);
    // Create the filter using the provided ruleData, service, subservice, limit and offset
    ...
    connection.v2.listEntities(filter).then(
        (response) => {
            ...
            if (response.count > limit + offset) {
                ...
                findSilentEntitiesByAPI(service, subservice, ruleData, func, callback, limit, offset + limit);
            } else {
