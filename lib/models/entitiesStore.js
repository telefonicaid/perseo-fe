    ...
    connection.v2.listEntities(filter).then(
        (response) => {
            ...
            if (response.count > filter.limit + filter.offset) {
                ...
                logger.info('%j entities found using findSilentEntitiesBy', filter.count);
            ...
            function(col, cb) {
                col.find(criterion)
                    ...
                    .each(function(err, one) {
                        ...
                        alterFunc(one);
                        entityCount++; // Increment the count of entities
                    });
            }
        ],
        function(err, result) {
            logger.debug('Total entities retrieved: %d', entityCount);
