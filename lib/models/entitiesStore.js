    ...
    connection.v2.listEntities(filter).then(
        (response) => {
            ...
            response.results.forEach((entity) => {
                logger.debug(contextAPI, 'silent entity %j', entity);
                alterFunc(entity);
            });
            ...
            if (response.count > filter.limit + filter.offset) {
                ...
                logger.info('%d entities found using findSilentEntitiesByAPI', response.results.lenght());
...
function findSilentEntitiesByMongo(service, subservice, ruleData, alterFunc, callback) {
    ...
    async.waterfall(
        ...
        function(err, result) {
            logger.info('%d entities found using findSilentEntitiesByMongo', entityCount);
...
function findSilentEntities(service, subservice, ruleData, alterFunc, callback) {
    ...
    var timedCallback = function(err, result) {
        ...
        logger.info('%s execution time (hr): %dms', method, hrend[1] / 1000000);
