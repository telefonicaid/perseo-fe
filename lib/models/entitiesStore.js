...
function createFilter(ruleData, service, subservice, limit, offset) {
    ...
    var filter = {
        ...
        mq:
            ruleData.attribute +
            '.dateModified<' +
            (Date.now() / 1000 - ruleData.reportInterval).toString() +
            ';' +
            'temperature.dateModified>' +
            (Date.now() / 1000 - maxTimeDetection).toString(),
        // TO DO: Limit query using entity maxTimeDetection if possible (#824)
                    ...
                    },
                    {
                        $addFields: {
                            ...
                            maxTimeDetection: {
                                $toDouble: {
                                    $ifNull: [
                                        '$attrs.maxTimeDetection.value',
                                        { $ifNull: [ruleData.maxTimeDetection, config.nonSignalMaxTimeDetection] }
                                    ]
                                }
                            },
                            reportInterval: { $toDouble: ruleData.reportInterval }
                        }
                    },
                    {
                        $match: {
