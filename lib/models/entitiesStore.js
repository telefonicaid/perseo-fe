        ...
        (response) => {
            ...
            if (response.results) {
                response.results.forEach((entity) => {
                    ...
                    alterFunc(entity);
                    accumulatedResults.push(entity); // Add the entity to the accumulated results
                });
            }
            ...
            if (response.count > filter.limit + filter.offset) {
                ...
                callback(null, accumulatedResults); // Return accumulated results instead of just the last page
            }
        },
        (error) => {
...
function findSilentEntitiesByAPI(service, subservice, ruleData, alterFunc, callback) {
    ...
    var accumulatedResults = []; // Add an array to store all results across multiple pages
...
function findSilentEntities(service, subservice, ruleData, alterFunc, callback) {
    ...
    var timedCallback = function(err, result, entityCount) {
        ...
        var totalEntities = !config.nonSignalByAPI ? entityCount : result.length;

        // Log the execution time in seconds and milliseconds
        logger.info('%s has found %d entities in (hr): %d ms', method, totalEntities, hrend[1] / 1000000);
