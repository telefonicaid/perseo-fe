...
function findSilentEntities(service, subservice, ruleData, alterFunc, callback) {
    ...
    var timedCallback = function(err, result, entityCount) {
        ...
        if (Array.isArray(result) && result.length > 0) {
            entitiesInfo = result.map(function(e) {
                var _id = e && e._id ? e._id : {};
                return {
                    id: _id.id,
                    service: context && context.service ? context.service : undefined,
                    subservice: _id.servicePath
                };
            });
        }
        ...
        logger.info(
            context,
            ' %s has found %d entities in (hr): %d ms entitiesInfo %j',
            method,
            entityCount,
            hrend[1] / 1000000,
            entitiesInfo
