...
function orionTenantDb(tenant) {
    return appContext.OrionDb(config.orionDb.prefix + '-' + tenant);
}

function findSilentEntities(tenant, service, ruleData, func, callback) {
    var db,
        criterion = {};

    db = orionTenantDb(tenant);
    criterion.attrs = {
        '$elemMatch': {
    ...
    criterion['_id.servicePath'] = service;
    if (ruleData.id) {
