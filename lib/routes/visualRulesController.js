...
function GetAllVR(req, resp) {
    ...
    visualRules.FindAll(req.service, req.subservice, function(err, data) {
        if (!err && util.isArray(data)) {
            var reformatData = data.map(function(e) {
                delete e.subservice;
                delete e.service;
                return e;
            });
            resp.set(constants.TOTAL_COUNT_HEADER, reformatData.length);
            reformatData = reformatData.splice(offset, limit);
        }
        myutils.respond(resp, err, reformatData, count);
    });
}
