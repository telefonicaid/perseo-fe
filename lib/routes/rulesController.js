...
function PutRules(req, resp) {
    req.body = req.body || {};
    req.body.subservice = req.subservice;
    req.body.service = req.service;
    logger.info({}, 'putting rule %j', req.body);
    metrics.IncMetrics(req.service, req.subservice, metrics.ruleUpdate);
    rules.Put(req.params.id, req.body, function(err, data) {
        if (err) {
            metrics.IncMetrics(req.service, req.subservice, metrics.failedRuleUpate);
        } else {
            metrics.IncMetrics(req.service, req.subservice, metrics.okRuleUpdate);
        }
        // Discard rules.Put intermediate results (del2core, noSignal.DeleteNSRul, postR2core, noSignal.AddNSRule)
        // keeping rulesStore.Update
        var res = data[data.length - 1];
        logger.debug({}, 'putRules res %j', res);
        myutils.respond(resp, err, res);
    });
}
...
function AddTo(app) {
     ...
     *         description: rules
     */
    app.put(config.endpoint.rulesPath + '/:id', PutRules);
}
