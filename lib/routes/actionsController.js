...
function PostAction(req, resp) {
    ...
    var PERSEO_MAX_RULES_WITH_SAME_CORR = 20; // TBD: take from config
    ...
    if (correlator) {
        ...
        if (util.isArray(rulesWithCorr)) {
            if (rulesWithCorr.length > config.maxRulesByCorr) {
                errMsg = util.format(
                    'maximun number of rules executed with the same correlator (%s) reached with these rules [%j] - The rule %s has not been triggered',
                    correlator,
                    rulesWithCorr,
                    firedEvent.ruleName
                );
